...
local MIGRATION_PATH = cutils.path:join(KONG_HOME, "database/migrations")
...
function Migrations:new(dao)
  ...
  self.migrations_files = cutils.retrieve_files(cutils.path:join(MIGRATION_PATH, dao.type), '.lua')
...
function Migrations.create(configuration, name, callback)
  for k, _ in pairs(configuration.databases_available) do
    ...
    local file_path = cutils.path:join(MIGRATION_PATH, k)
...
function Migrations:rollback(callback)
  ...
  if old_migrations and #old_migrations > 0 then
    migration_to_rollback = loadfile(cutils.path:join(MIGRATION_PATH, self.dao.type, old_migrations[#old_migrations])..".lua")()
  else
