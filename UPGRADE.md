...
- The required OpenResty version has been bumped to 1.15.8.1. Kong's
  [openresty-patches](https://github.com/kong/openresty-patches) and
  the newly introduced [lua-kong-nginx-module](https://github.com/Kong/lua-kong-nginx-module)
  are **required** for Kong to work properly.
- The minimum required OpenSSL version is 1.1.1 (same with 1.2.x).
  If you are building by hand, make sure all dependencies,
  including LuaRocks modules, are compiled using the same OpenSSL version.
...
In addition, Kong 1.0 included a number of breaking changes over Kong 0.x. If you are upgrading
...
1. Download 1.3, and configure it to point to the same datastore
   as your old (1.0 - 1.2) cluster. Run `kong migrations up`.
2. Once that finishes running, both the old and new (1.3) clusters can now
   run simultaneously on the same datastore. Start provisioning
   1.3 nodes, but do not use their Admin API yet. If you need to
...
3. Gradually divert traffic away from your old nodes, and into
   ...
   is going smoothly.
4. When your traffic is fully migrated to the 1.3 cluster,
   decommission your old nodes.
5. From your 1.3 cluster, run: `kong migrations finish`.
