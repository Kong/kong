...
Kong 1.4.0rc1 does not include any breaking changes over Kong 1.3, but Kong 1.3
...
##### Upgrade from `0.x` to `1.4.0rc1`

The lowest version that Kong 1.4.0rc1 supports migrating from is 0.14.1. if you
are migrating from a previous 0.x release, please migrate to 0.14.1 first.

For upgrading from 0.14.1 to Kong 1.4.0rc1, the steps for upgrading are the same
...
##### Upgrade from `1.0.x` - `1.3.x` to `1.4.0rc1`

Kong 1.4.0rc1 supports a no-downtime migration model. This means that while the
...
database in the final expected state for Kong 1.4.0rc1).

1. Download 1.4.0rc1, and configure it to point to the same datastore
   as your old (1.0 to 1.3) cluster. Run `kong migrations up`.
2. Once that finishes running, both the old and new (1.4.0rc1) clusters can now
   run simultaneously on the same datastore. Start provisioning
   1.4.0rc1 nodes, but do not use their Admin API yet. If you need to
...
3. Gradually divert traffic away from your old nodes, and into
   your 1.4.0rc1 cluster. Monitor your traffic to make sure everything
   is going smoothly.
4. When your traffic is fully migrated to the 1.4.0rc1 cluster,
   decommission your old nodes.
5. From your 1.4.0rc1 cluster, run: `kong migrations finish`.
   ...
   requests to your 1.4.0rc1 nodes.

##### Installing 1.4.0rc1 on a Fresh Datastore

The following commands should be used to prepare a new 1.4.0rc1 cluster from a
fresh datastore:
