...
If you are migrating from any 2.x series, upgrading into 2.4.x is a minor upgrade,
...
[1.19.3.1](http://openresty.org/en/changelog-1019003.html). This is more recent
...
There is a new way to deploy Go using Plugin Servers. You can read more about it the [Go section of the docs](https://docs.konghq.com/2.4.x/go).
...
Here's how to see the differences between 2.0.0 and 2.4.0:
...
##### Upgrade from `0.x` to `2.4.0`

The lowest version that Kong 2.4.0 supports migrating from is 1.0.0.
...
below to migrate to 2.4.0.

##### Upgrade from `1.0.0` - `2.3.0` to `2.4.0`

**Postgres**

Kong 2.4.0 supports a no-downtime migration model. This means that while the
...
database in the final expected state for Kong 2.4.0).

1. Download 2.4.0, and configure it to point to the same datastore
   as your old (1.0 to 2.0) cluster. Run `kong migrations up`.
2. Once that finishes running, both the old (pre-2.4) and new (2.4.0)
   clusters can now run simultaneously. Start provisioning 2.4.0 nodes,
...
3. Gradually divert traffic away from your old nodes, and into
   your 2.4.0 cluster. Monitor your traffic to make sure everything
   is going smoothly.
4. When your traffic is fully migrated to the 2.4.0 cluster,
   decommission your old nodes.
5. From your 2.4.0 cluster, run: `kong migrations finish`.
   ...
   requests to your 2.4.0 nodes.

**Cassandra**

Due to internal changes, the table schemas used by Kong 2.4.0 on Cassandra
...
1. Download 2.4.0, and configure it to point to a new keyspace.
...
5. Gradually divert traffic away from your old nodes, and into
   your 2.4.0 cluster. Monitor your traffic to make sure everything
   is going smoothly.
6. When your traffic is fully migrated to the 2.4.0 cluster,
   decommission your old nodes.

##### Installing 2.4.0 on a Fresh Datastore

The following commands should be used to prepare a new 2.4.0 cluster from a
