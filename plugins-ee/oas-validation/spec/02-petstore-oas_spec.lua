    ...
    validation_plugin = [[
      server {
          ...
          location ~ "/store/order-good" {
            content_by_lua_block {
              local body = require("pl.file").read("]] .. helpers.get_fixtures_path() .. [[/petstore-store-order-response.json")
              ngx.status = 200
              ngx.header["Content-Type"] = "application/json"
              ngx.header["Content-Length"] = #body
              ngx.print(body)
          ...
          location ~ "/store/order-bad" {
            content_by_lua_block {
              local body = require("pl.file").read("]] .. helpers.get_fixtures_path() .. [[/petstore-store-order-invalid-response.json")
              ngx.status = 200
              ngx.header["Content-Type"] = "application/json"
              ngx.header["Content-Length"] = #body
              ngx.print(body)
  ...
  describe(PLUGIN_NAME .. ": (access) [#" .. strategy .. "]", function()
    ...
    lazy_setup(function()
      ...
      db.plugins:insert {
        ...
        route = { id = route1.id },
        config = {
          api_spec = assert(io.open(helpers.get_fixtures_path() .. "/resources/petstore-oas.json"):read("*a")),
