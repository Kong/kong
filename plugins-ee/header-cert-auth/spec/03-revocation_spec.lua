...
cd plugins-ee/header-cert-auth/spec/fixtures/ocsp-responder-docker
...
local function pem_to_leaf_only_string(path)
  local pem = pl_file.read(path)

  local out = ngx_re_gmatch(pem,
...
for _, strategy in strategies() do
  describe("Plugin: header-cert-auth (revocation) [#" .. strategy .. "]", function()
    local mtls_client
    local bp, db
    local service
    ...
    lazy_setup(function()
      ...
      mock = http_mock.new(HTTP_SERVER_PORT, {
        ["/no_proxy_strict_ocsp_valid"] = {
          directives = fmt(format, "noproxy-strict", "noproxy-strict"),
        },
        ...
        ["/no_proxy_skip_ocsp_valid_inter"] = {
          directives = fmt(format, "noproxy-skip", "noproxy-skip"),
        },
        ...
        ["/proxy_strict_ocsp_valid_inter"] = {
          directives = fmt(format, "proxy-strict", "proxy-strict"),
        },
        ...
        ["/bad_proxy_strict_ocsp_valid_inter"] = {
          directives = fmt(format, "badproxy-strict", "badproxy-strict"),
        },
        ...
        ["/bad_proxy_ignore_ocsp_valid_inter"] = {
          directives = fmt(format, "badproxy-ignore", "badproxy-ignore"),
        },
    ...
    for _, case in ipairs(test_cases) do
      ...
      if res1 then
        ...
        it(fmt("returns HTTP %s on https request if %s certificate passed with no proxy (%s, leaf_only = %s, mode = %s), cache miss for revocation status",
          ...
          local path = helpers.get_fixtures_path() .. "/ocsp-responder-docker/certificates/%s-%s.pem"
          local cert_leaf = pem_to_leaf_only_string(fmt(path, case_type, cert))
          local res = assert(mtls_client:send {
            method  = "GET",
            path    = url,
            headers = {
              ["ssl-client-cert"] = cert_leaf,
            },
          })
