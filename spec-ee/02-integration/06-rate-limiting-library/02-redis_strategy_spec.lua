...
describe("rate-limiting: Redis strategy with vault reference", function()
  local strategy, redis_client
  ...
  local redis_opts = {
    ...
    database = 0,
    password = "{vault://env/vault-test-redis-password}",
    ["$refs"] = {
      password = "{vault://env/vault-test-redis-password}"
    }
  }
  ...
  describe(":vault referenced password", function()
    it("connects to redis using vault", function()
      local test_diff = {
        {
          key     = "foo",
          windows = {
            {
              namespace = "not_default_workspace",
              window    = mock_start,
              size      = mock_window_size,
              diff      = 1,
            },
      ...
      local _, err = strategy:push_diffs(test_diff)
      assert.is_not_nil(err)

      helpers.setenv("VAULT_TEST_REDIS_PASSWORD", "passdefault")
