...
for _, strategy in helpers.each_strategy() do
  ...
  it = bu.test_with_prefixes(_it, strategy, {"ws1", "ws2"}) -- luacheck: ignore
  ...
  describe("Healthcheck #" .. strategy, function()
    lazy_setup(function()
      ...
      if strategy ~= "off" then
        bp.workspaces:insert({ name = "ws1" })
        bp.workspaces:insert({ name = "ws2" })
      end
        ...
        describe("#" .. mode, function()
          for _, consistency in ipairs(bu.consistencies) do
            describe("Upstream entities #" .. consistency, function()
              ...
              it("#db can be renamed without producing stale cache", function()
                ...
                for i = 1, 2 do
                  upstreams[i] = {}
                  local name, id = bu.add_upstream(bp, {
                    healthchecks = bu.healthchecks_config {}
                  })
                  upstreams[i].name = name
                  upstreams[i].id = id
                ...
                bu.remove_upstream(bp, upstreams[1].id)
                bu.remove_upstream(bp, upstreams[2].id)
              end)
          ...
          describe("#healthchecks", function()
            ...
            it("do not count Kong-generated errors as failures", function()
              ...
              local api_host, service_id = bu.add_api(bp, upstream_name, { retries = 1 })
