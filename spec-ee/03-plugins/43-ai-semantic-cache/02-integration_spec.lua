...
local TEST_SCANARIOS = {
  ...
  { id = "ea9513a1-6c5a-44f2-b60f-df083c1b036b", desc = "bad embeddings response",      vector_config = "good", embeddings_config = "good",                   embeddings_response = "bad",  chat_request = "good", stop_on_failure = true, message_countback = 10, expect = 500 },
  { id = "43df7fd8-6c1e-4ff4-902f-84a655196679", desc = "bad inference request",        vector_config = "good", embeddings_config = "good",                   embeddings_response = "good", chat_request = "bad",  stop_on_failure = true, message_countback = 10, expect = 400 },
}
          ...
          for _, TEST_SCENARIO in ipairs(TEST_SCANARIOS) do
            ...
            if TEST_SCENARIO.with_ai_proxy then
              bp.plugins:insert {
                ...
                config = {
                  model = {
                    name = TEST_SCENARIO.model or "gpt4",
                    provider = "openai",
                    options = {
                      input_cost = 100,
                      output_cost = 100,
              ...
              config = {
                ...
                embeddings = {
                  ...
                  model = {
                    ...
                    options = {
                      upstream_url = "http://"..helpers.mock_upstream_host..":"..MOCK_PORT.."/embeddings/" .. TEST_SCENARIO.embeddings_response,
                    }
                  }
                },
                vectordb = {
      ...
      for _, EMBEDDINGS_STRATEGY in ipairs(COMPATIBLE_EMBEDDINGS) do
          for _, TEST_SCENARIO in ipairs(TEST_SCANARIOS) do
            ...
            it("[#" .. VECTORDB_STRATEGY.. "] [#" .. EMBEDDINGS_STRATEGY .. "] " .. TEST_SCENARIO.desc, function()
              ...
              local vectordb_namespace_suffix = TEST_SCENARIO.with_ai_proxy and "openai" or "UNSPECIFIED"
              local model_name = TEST_SCENARIO.model or "gpt4"
              vectordb_namespace_suffix = vectordb_namespace_suffix .. "-" .. model_name

              local vector_connector = vectordb.new(VECTORDB_STRATEGY, fmt("kong_semantic_cache:%s:%s", TEST_SCENARIO.id, vectordb_namespace_suffix), {
                strategy = VECTORDB_STRATEGY,
                dimensions = DIMENSIONS_LOOKUP[TEST_SCENARIO.embeddings_config],
                distance_metric = "cosine",
                threshold = 0.7,
              ...
              local original_request = pl_file.read(fmt("spec-ee/fixtures/ai-proxy/chat/request/%s.json", TEST_SCENARIO.chat_request))
              original_request = cjson.decode(original_request)
              -- as here we are testing without ai-proxy, set one in the request model to to cover the codepath of prefix naming
              -- (by default the plugin uses UNSPECIFIED)
              original_request.model = model_name

              local r = proxy_client:post(fmt("/%s/%s/%s", VECTORDB_STRATEGY, EMBEDDINGS_STRATEGY, TEST_SCENARIO.id), {
                headers = {
              ...
              if TEST_SCENARIO.expect == 200 then
                ...
                -- keys
                local ai_node = decoded.ai and decoded.ai["proxy"]
                ...
                if not TEST_SCENARIO.with_ai_proxy then
                  assert.is_nil(ai_node.usage)
                else
                  assert.is_not_nil(ai_node.usage)
                end
              ...
              elseif TEST_SCENARIO.expect >= 400 then
                if TEST_SCENARIO.embeddings_config == "bad_too_few_dimensions" then
                  local body = assert.res_status(TEST_SCENARIO.expect, r)
                  body = cjson.decode(body)

                  assert.match("Embedding dimensions do not match the configured vector database", body.message)
                else
                  assert.res_status(TEST_SCENARIO.expect, r)
                end

                return
              end

              wait_until_key_in_cache(vector_connector, "kong_semantic_cache:" .. TEST_SCENARIO.id .. ":" .. vectordb_namespace_suffix .. ":*")
              ...
              if TEST_SCENARIO.stream_response then
                ...
                -- full URI.
                local stream_request = deep_copy(original_request)
                ...
                local res, err = httpc:request({
                    ...
                    method = "POST",
                    body = cjson.encode(stream_request),
                })
                if not res then
                  assert.is_nil(err)
                end
                ...
                local new_body_to_send = deep_copy(original_request)
                if TEST_SCENARIO.append_message then
                  table.insert(new_body_to_send.messages, {
                    role = "user",
                    content = "what is 1 + 1?"
                  })
                end
                ...
                if TEST_SCENARIO.append_message then
                  ...
                  r = proxy_client:post(fmt("/%s/%s/%s", VECTORDB_STRATEGY, EMBEDDINGS_STRATEGY, TEST_SCENARIO.id), {
                    headers = {
                      ["content-type"] = "application/json",
                      ["accept"] = "application/json",
                      ["authorization"] = "Bearer openai-key",
