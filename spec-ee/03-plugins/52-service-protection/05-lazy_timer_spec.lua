...
for _, strategy in helpers.all_strategies() do
  describe(fmt("%s - lazy timer [#%s]", plugin_name, strategy), function()
    ...
    lazy_setup(function()
      ...
      assert(bp.routes:insert({ paths = { "/test" } }))
    ...
    it("destroy the timer when the plugin is deleted", function()
      ...
      assert.logfile().has.line("clearing old namespace foo", true, 10)
      assert.logfile().has.line("stale timer of namespace foo", true, 10)
    end)
  end)
end
