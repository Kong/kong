...
# simple environment with cassandra and postgres database servers and a Kong node.
...
function run_tests() {
    # Run the tests
    BUSTED_ENV="env KONG_DATABASE=$1 KONG_DNS_RESOLVER= KONG_TEST_PG_DATABASE=kong KONG_TEST_CASSANDRA_KEYSPACE=kong OLD_KONG_VERSION=$OLD_KONG_VERSION"
