...
def common_suites(expect, libxcrypt_no_obsolete_api: bool = False, skip_libsimdjson_ffi: bool = False):
    ...
    expect("/usr/local/openresty/nginx/sbin/nginx", "nginx compiled with OpenSSL 3.5.x") \
        .nginx_compiled_openssl.matches(r"OpenSSL 3.5.\d") \
        .version_requirement.key("libssl.so.3").less_than("OPENSSL_3.6.0") \
        .version_requirement.key("libcrypto.so.3").less_than("OPENSSL_3.6.0") \

    expect("**/*.so", "dynamic libraries are compiled with OpenSSL 3.5.x") \
        .version_requirement.key("libssl.so.3").less_than("OPENSSL_3.6.0") \
        .version_requirement.key("libcrypto.so.3").less_than("OPENSSL_3.6.0") \
