...
function _M:handle_cp_websocket(cert)
  ...
  if not self.deflated_reconfigure_payload then
    ...
    local ok
    ok, err, sync_status = self:check_configuration_compatibility(dp, self.reconfigure_payload.config_table)
    if not ok then
      update_sync_status()
    end
  end
  ...
  local write_thread = ngx.thread.spawn(function()
    while not exiting() do
      ...
      ok, err, sync_status = self:check_configuration_compatibility(
                                dp, self.reconfigure_payload.config_table)
      -- XXX EE ]]
      if not ok then
      ...
      -- XXX EE [[
      local _, deflated_payload, err = update_compatible_payload( self.reconfigure_payload, dp_version, log_suffix, self.plugins_map, dp_plugins_map)
      -- XXX EE ]]
