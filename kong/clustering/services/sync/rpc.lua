...
local function paged_full_sync_payload(page, next_token)
  return {
    default = {
      full_sync = true,
      deltas = page,
      next_token = next_token and assert(json_encode(next_token)),
    },
  }
end
...
local function full_sync(self, workspace)
  ...
  local next_token = workspace.next_token
...
function _M:init_cp(manager)
  ...
  manager.callbacks:register("kong.sync.v2.get_delta", function(node_id, current_versions)
    ...
    if default_namespace_version == 0 or
       ...
       default_namespace.next_token -- a full-sync session is in progress
    then
...
local function do_sync()
  ...
  if payload.full_sync then
    full_sync = true
    first_page = not next_token and payload.next_token
    last_page = not payload.next_token
  ...
  if full_sync then
    ...
    if last_page then
      ...
      kong_shm:set(CLUSTERING_DATA_PLANES_PAGED_FULL_SYNC_NEXT_TOKEN_KEY, payload.next_token)
