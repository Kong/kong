...
function _INJECTOR:new(e)
  ...
  assert(type(inst.trace_id_allowed_sizes) == "table" and
         ...
         err .. "invalid trace_id_allowed_sizes parameter")

  assert(type(inst.span_id_size_bytes) == "number" and
         inst.span_id_size_bytes > 0,
         err .. "invalid span_id_size_bytes parameter")
...
function _INJECTOR:verify_any(context)
  if not self.context_validate.any or next(self.context_validate.any) == nil then
    return true
  end

  for _, field in ipairs(self.context_validate.any) do
    if context and type(context) == "table" and context[field] ~= nil then
      return true
    end
  end

  return false, "no required field found in context: " ..
                table.concat(self.context_validate.any, ", ")
end


function _INJECTOR:verify_all(context)
  if not self.context_validate.all or next(self.context_validate.all) == nil then
    return true
  end

  for _, field in ipairs(self.context_validate.all) do
    if not context or type(context) ~= "table" or context[field] == nil then
...
function _INJECTOR:verify_context(context)
  if not context then
    return false, "no context to verify"
  end
...
function _INJECTOR:inject(inj_tracing_ctx)
  local context_valid, err = self:verify_context(inj_tracing_ctx)
  ...
  -- Use trace_id_allowed_sizes to try to keep the original (incoming)
  -- size where possible.
