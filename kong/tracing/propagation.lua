...
local function set(conf_header_type, found_header_type, proxy_span, conf_default_header_type)
  ...
  if conf_header_type == "datadog" or found_header_type == "datadog" then
    local trace_id = from_binary(proxy_span.trace_id:sub(-8)):to_dec()
    trace_id_formats.datadog = trace_id

    set_header("x-datadog-trace-id", trace_id)
  ...
  if conf_header_type == "gcp" or found_header_type == "gcp" then
    ...
    set_header("x-cloud-trace-context", trace_id ..
      "/" .. from_binary(proxy_span.span_id):to_dec() ..
