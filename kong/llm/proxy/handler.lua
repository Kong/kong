...
function _M:header_filter(conf)
  ...
  if llm_state.is_response_transformer_skipped() or llm_state.is_semantic_cache_hit() then
    return
  end
  ...
  if not llm_state.is_streaming_mode() then
    ...
    if route_type == "preserve" then
      llm_state.set_parsed_response(response_body)
    else
      ...
      if err then
        ...
        ngx.status = 500
        llm_state.set_parsed_response(cjson.encode({ error = { message = err } }))
      ...
      elseif new_response_string then
        ...
        llm_state.set_parsed_response(new_response_string)
      end
    end
  end
...
function _M:body_filter(conf)
  ...
  if llm_state.get_semantic_cache_hit() then
  ...
  if not llm_state.is_response_transformer_skipped() then
    ...
    if route_type ~= "preserve" then
      if llm_state.is_streaming_mode() then
        ...
        local original_request = llm_state.get_parsed_response()
