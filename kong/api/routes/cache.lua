...
local function get_cache(self)
  self.params.cache = caches[ngx.req.get_uri_args()["cache"] or "lua"]
end
  ...
  ["/cache/:key"] = {
    ...
    GET = function(self, dao_factory)
      if self.params.key then
        local cached_item = self.params.cache("get", self.params.key)
        if cached_item then
    ...
    DELETE = function(self, dao_factory)
      if self.params.key then
        self.params.cache("delete", self.params.key)
        return responses.send_HTTP_NO_CONTENT()
      end
