...
local function handle_put_log_level(self, broadcast)
  ...
  if type(log_level) ~= "number" then
    return kong.response.exit(400, { message = "unknown log level" })
  end
  ...
  return kong.response.exit(201, { message = "log level changed" })
end
...
local routes = {
  ...
  ["/debug/node/log-level/:log_level"] = {
    PUT = function(self)
      return handle_put_log_level(self, false)
    end
  },
}
...
routes[cluster_name] = {
  PUT = function(self)
    return handle_put_log_level(self, true)
  end
}
