...
local function parse_phase_filter(x_kong_request_debug)
  --[[
  ...
  -- match string like "rewrite,access,header_filter"
  local itor, err = ngx.re.gmatch(x_kong_request_debug, "([a-z_]+)", "jo")
  if not itor then
    ngx.log(ngx.ERR, "failed to match X-Kong-Request-Debug: ", err)
    return nil
  end

  local filter = {}
  for phase in itor do
    if VALID_PHASES[phase[1]] then
      filter[phase[1]] = true

    else
      -- invalid phase, disabled this feature
      return nil
    end
  end
...
function _M.auth()
  ...
  local filter = parse_phase_filter(http_x_kong_request_debug)
  if not filter then
    return
  end

  local ctx = context.new(filter, {
