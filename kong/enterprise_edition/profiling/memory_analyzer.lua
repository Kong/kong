...
local ma                        = require "jit.mem_analyze"

local ngx_time                  = ngx.time
local ngx_worker_pid            = ngx.worker.pid

local SHDICT_STATE              = "kong_profiling_state"
local SHDICT_LOCK               = "kong_locks"
local LOCK_KEY                  = "memory-analyzer:lock"
local STATUS_KEY                = "memory-analyzer:status"
local PID_ID_KEY                = "memory-analyzer:pid"
local TIMEOUT_AT_KEY            = "memory-analyzer:timeout_at"
local PATH_KEY                  = "memory-analyzer:path"
local ERROR_KEY                 = "memory-analyzer:error"
...
local function get_shdict()
  return assert(ngx.shared[SHDICT_STATE])
end
...
function _M.analyze(path, timeout)
  if _M.is_active() then
    return nil, "another memory analyzer is running"
  end

  mark_active(path, timeout)

  --[[
    ok = ma.analyzer(raw_dump_file_path[, timeout=120])

    This function will trigger a full GC cycle, and then dump the Lua VM heap into raw flamegraph format.
