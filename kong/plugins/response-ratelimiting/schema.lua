...
return {
  ...
  fields = {
    ...
    { config = {
        type = "record",
        fields = {
          { header_name = { description = "The name of the response header used to increment the counters.", type = "string", default = "x-kong-limit" }, },
          { limit_by = { description = "The entity that will be used when aggregating the limits: `consumer`, `credential`, `ip`. If the `consumer` or the `credential` cannot be determined, the system will always fallback to `ip`.", type = "string",
                         default = "consumer",
          ...
          { redis_host = typedefs.host, description = "When using the `redis` policy, this property specifies the address to the Redis server." },
          { redis_port = typedefs.port({ default = 6379, description = "When using the `redis` policy, this property specifies the port of the Redis server." }), },
          ...
          { redis_username = { description = "When using the `redis` policy, this property specifies the username to connect to the Redis server when ACL authentication is desired.\nThis requires Redis v6.0.0+. The username **cannot** be set to `default`.", type = "string", referenceable = true }, },
          ...
          { redis_server_name = typedefs.sni, description= "When using the `redis` policy with `redis_ssl` set to `true`, this property specifies the server name for the TLS extension Server Name Indication (SNI) " },
          ...
          { redis_database = { description = "When using the `redis` policy, this property specifies Redis database to use.", type = "number", default = 0 }, },
          { block_on_first_violation = { description = "A boolean value that determines if the requests should be blocked as soon as one limit is being exceeded. This will block requests that are supposed to consume other limits too.", type = "boolean", required = true, default = false }, },
