  ...
  fields = {
    ...
    { config = {
        ...
        fields = {
          ...
          {
            lock_dictionary_name = { description = "The shared dictionary where concurrency control locks are stored. The default shared dictionary is `kong_locks`. The shared dictionary should be declared in nginx-kong.conf.", type = "string",
            default = "kong_locks",
            required = true,
          }},
      ...
      fn = function(entity)
        ...
        if config.strategy == "cluster" and config.sync_rate ~= -1 then
          if kong.configuration.role ~= "traditional" or kong.configuration.database == "off" then
            return nil, concat{ "[service-protection] ",
