...
=== TEST 3: request.get_scheme() considers X-Forwarded-Proto when trusted
--- SKIP: TODO if we include this logic there
--- config
    location = /t {
        content_by_lua_block {
            local SDK = require "kong.sdk"
            local sdk = SDK.new()
            sdk.init({ trusted_ips = { "0.0.0.0/0" } }, "ip")
...
=== TEST 4: request.get_scheme() considers first X-Forwarded-Proto if multiple when trusted
--- SKIP: TODO if we include this logic there
--- config
    location = /t {
        content_by_lua_block {
