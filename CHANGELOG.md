...
usability of Kong! As usual, expect a few migrations to be run as well before
upgrading to Kong 0.12.0.
...
before planning to upgrade your Kong cluster if you happen to run an anterior
version.
...
- :warning: By default, the Admin API now only listens on the local interface.
  ...
  Beware of this change if you are using Kong in a containerized environment,
  as you may have to ensure your Admin API is reachable via the host's
  interface.
