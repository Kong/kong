...
- :warning: The Consumers entities have moved to the new DAO implementation. As
  such, the `/consumers` and `/snis` endpoint has received notable
  usability improvements, but suffers from a few breaking changes.
...
- :fireworks: Kong now ships with OpenSSL 1.1.0.
  [#3498](https://github.com/Kong/kong/pull/3498)
...
- :fireworks: Support for `PUT` in new endpoints (e.g. `/services/{id or
  name}`, `/routes/{id}`, `/consumers/{id}`), allowing the development of
  idempotent configuration workflow when scripting the Admin API.
