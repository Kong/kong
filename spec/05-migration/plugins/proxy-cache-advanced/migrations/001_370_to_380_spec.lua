...
if uh.database_type() == 'postgres' then
    describe("proxy-cache-advanced plugin migration - timeout field", function()
        ...
        describe("when timeout field was defined", function()
            ...
            uh.setup(function ()
                ...
                assert.equal("proxy-cache-advanced", body.name)
            ...
            uh.new_after_finish("has updated proxy-cache-advanced redis configuration - connect,read,send are present", function ()
                local admin_client = assert(uh.admin_client())
                local res = assert(admin_client:send {
                    method = "GET",
                    path = "/routes/" .. route1_name .. "/plugins/",
                ...
                assert.equal("proxy-cache-advanced", body.data[1].name)
