...
if uh.database_type() == 'postgres' then
  describe("openid-connect plugin migration - move to shared redis schema", function()
    local route1_name = "test1"
    local route2_name = "test2"
    local route3_name = "test3"

    describe("when openid-connect is configured - single Redis instance", function()
      lazy_setup(function()
      ...
      local handler28 = uh.get_busted_handler("2.8.x.x", "3.3.x.x")
      local handler34 = uh.get_busted_handler("3.4.x.x")
      handler34("with kong version 3.4.x.x -- (one that includes session_redis_cluster_max_redirections)", function ()
        uh.setup(function ()
          ...
          assert.equal("openid-connect", body.name)
          assert.same('https://accounts.google.test/.well-known/openid-configuration', body.config.issuer)
        ...
        uh.new_after_finish("has updated openid-connect redis configuration - session_redis_* fields have been moved to redis.*", function ()
            local admin_client = assert(uh.admin_client())
            local res = assert(admin_client:send {
                method = "GET",
                path = "/routes/" .. route1_name .. "/plugins/",
            ...
            assert.equal("openid-connect", body.data[1].name)
            assert.same('https://accounts.google.test/.well-known/openid-configuration', body.data[1].config.issuer)
      ...
      handler28("with kong version 2.8.x.x -- (one that does not include session_redis_cluster_max_redirections)", function ()
        uh.setup(function ()
          local admin_client = assert(uh.admin_client())
          local res = assert(admin_client:send {
            method = "POST",
    ...
    describe("when openid-connect is configured - cluster", function()
      local cluster_nodes = {
    ...
    describe("when openid-connect is configured - both cluster and single redis instance", function()
      local cluster_nodes = {
        { ip = '127.0.0.1', port = 6379 },
        { ip = '127.0.0.1', port = 6380 },
        { ip = '127.0.0.1', port = 6381 },
      ...
      uh.new_after_finish("has updated openid-connect redis configuration - only redis cluster fields were migrated", function ()
          local admin_client = assert(uh.admin_client())
          local res = assert(admin_client:send {
              method = "GET",
              path = "/routes/" .. route3_name .. "/plugins/",
