...
if uh.database_type() == 'postgres' then
    describe("graphql-rate-limiting-advanced plugin migration - timeout field", function()
        ...
        describe("when timeout field was defined", function()
            ...
            uh.setup(function ()
                ...
                assert.equal("graphql-rate-limiting-advanced", body.name)
            ...
            uh.new_after_finish("has updated graphql-rate-limiting-advanced redis configuration - connect,read,send are present", function ()
                local admin_client = assert(uh.admin_client())
                local res = assert(admin_client:send {
                    method = "GET",
                    path = "/routes/" .. route1_name .. "/plugins/",
                ...
                assert.equal("graphql-rate-limiting-advanced", body.data[1].name)
