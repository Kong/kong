...
describe("Router", function()
  ...
  describe("select()", function()
    ...
    describe("[headers]", function()
      ...
      describe("[websockets]", function()
        local ws_service     = { name = "ws-service",     protocol = "ws" }
        local non_ws_service = { name = "non-ws-service", protocol = "http" }

        local use_case = {
          {
            service = non_ws_service,
            route = {
              name      = "maybe-ws",
              hosts     = { "test" },
              paths     = { "/other" },
        ...
        local function find_route(method, path, version, headers)
          headers = setmetatable(headers or {}, headers_mt)
          return router.select(method, path, "test", "http",
                               nil, nil, nil, nil, nil,
                               headers, version)
        end

        it("websocket routes are matched with higher priority than http", function()
          local match_t = find_route("GET", "/test", 1.1, {
                                       connection = "Upgrade",
                                       upgrade = "websocket"
                                     })

          assert.truthy(match_t)
          assert.equals("ws", match_t.route.name)
        end)

        it("non-websocket handshake requests do not match websocket routes", function()
          local match_t = find_route("GET", "/test")
          assert.truthy(match_t)
          assert.equals("non-ws", match_t.route.name)
        end)

        it("websocket handshake requests can match non-websocket routes", function()
          local match_t = find_route("GET", "/other", 1.1, {
                                       connection = "Upgrade",
                                       upgrade = "websocket"
                                     })
          assert.truthy(match_t)
          assert.equals("maybe-ws", match_t.route.name)
        end)

        it("websocket routes must match all websocket handshake criteria", function()
          local requests = {
            {
            ...
            {
              description = "invalid upgrade header",
              method = "GET",
              version = 1.1,
          ...
          for _, req in ipairs(requests) do
            ...
            local match_t = find_route(req.method, "/ws", req.version, headers)
            assert.falsy(match_t, "expected non-match for " .. req.description)
          end
  ...
  describe("exec()", function()
    ...
    local function mock_ngx(method, request_uri, headers, http_version)
      local _ngx
      _ngx = {
        re = ngx.re,
        var = setmetatable({
