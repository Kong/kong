...
describe("Configuration loader", function()
  it("loads the defaults", function()
    ...
    assert.same({"0.0.0.0:8000", "0.0.0.0:8443 ssl"}, conf.listen)
  ...
  it("loads a given file, with higher precedence", function()
    ...
    assert.same({"0.0.0.0:9000", "0.0.0.0:9443 ssl"}, conf.listen)
    assert.is_nil(getmetatable(conf))
  end)
  ...
  it("extracts flags, ports and listen ips from listen/admin_listen", function()
    local conf = assert(conf_loader())
    ...
    assert.equal("0.0.0.0", conf.listeners[1].ip)
    assert.equal(8000, conf.listeners[1].port)
    assert.equal(false, conf.listeners[1].ssl)
    assert.equal(false, conf.listeners[1].http2)
    assert.equal("0.0.0.0:8000", conf.listeners[1].listener)

    assert.equal("0.0.0.0", conf.listeners[2].ip)
    assert.equal(8443, conf.listeners[2].port)
    assert.equal(true, conf.listeners[2].ssl)
    assert.equal(false, conf.listeners[2].http2)
    assert.equal("0.0.0.0:8443 ssl", conf.listeners[2].listener)
  end)
  it("extracts ssl flags properly when hostnames contain them", function()
    local conf
    conf = assert(conf_loader(nil, {
      listen = "ssl.myname.com:8000",
      admin_listen = "ssl.myname.com:8001",
    }))
    assert.equal("ssl.myname.com", conf.listeners[1].ip)
    assert.equal(false, conf.listeners[1].ssl)
    assert.equal("ssl.myname.com", conf.admin_listeners[1].ip)
    assert.equal(false, conf.admin_listeners[1].ssl)

    conf = assert(conf_loader(nil, {
      listen = "ssl_myname.com:8000 ssl",
      admin_listen = "ssl_myname.com:8001 ssl",
    }))
    assert.equal("ssl_myname.com", conf.listeners[1].ip)
    assert.equal(true, conf.listeners[1].ssl)
    assert.equal("ssl_myname.com", conf.admin_listeners[1].ip)
    assert.equal(true, conf.admin_listeners[1].ssl)
  end)
  it("extracts 'off' from listen/admin_listen", function()
    local conf
    conf = assert(conf_loader(nil, {
      listen = "off",
      admin_listen = "off",
    }))
    assert.same({}, conf.listeners)
    assert.same({}, conf.admin_listeners)
    -- off with multiple entries
    conf = assert(conf_loader(nil, {
      listen = "off, 0.0.0.0:9000",
    ...
    conf = assert(conf_loader(nil, {
      listen = "offshore.com:9000",
      admin_listen = "offshore.com:9001",
    }))
    assert.same("offshore.com", conf.listeners[1].ip)
  ...
  describe("validations", function()
    ...
    it("enforces listen addresses format", function()
      ...
      assert.is_nil(conf)
      assert.equal("listen must be of form: [off] | <ip>:<port> [ssl] [http2] [proxy_protocol], [... next entry ...]", err)
    end)
