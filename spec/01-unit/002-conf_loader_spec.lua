...
describe("Configuration loader", function()
  ...
  it("extracts ports and listen ips from proxy_listen/admin_listen", function()
    ...
    assert.equal("0.0.0.0", conf.portal_gui_ip)
    assert.equal(8003, conf.portal_gui_port)
    assert.equal("0.0.0.0", conf.portal_gui_ssl_ip)
    assert.equal(8446, conf.portal_gui_ssl_port)
    assert.equal("0.0.0.0", conf.portal_api_ip)
    assert.equal(8004, conf.portal_api_port)
    assert.equal("0.0.0.0", conf.portal_api_ssl_ip)
    assert.equal(8447, conf.portal_api_ssl_port)
  end)
  ...
  describe("validations", function()
    ...
    it("enforces listen addresses format", function()
      ...
      conf, err = conf_loader(nil, {
        portal = "on",
        portal_api_listen_ssl = "127.0.0.1"
      })
      assert.is_nil(conf)
