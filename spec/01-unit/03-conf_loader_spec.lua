...
describe("Configuration loader", function()
  ...
  describe("#wasm properties", function()
    local function search_filter(tbl, filter_name, filter_path)
      for _, filter in pairs(tbl) do
        if filter.name == filter_name and filter.path == filter_path then
          return true
        end
      end
      return false
    ...
    it("wasm_filters_path", function()
      ...
      assert.True(search_filter(conf.wasm_modules_parsed, "tests", 
                  "spec/fixtures/proxy_wasm_filters/target/wasm32-wasi/debug/tests.wasm"))
      assert.True(search_filter(conf.wasm_modules_parsed, "response_transformer", 
                  "spec/fixtures/proxy_wasm_filters/target/wasm32-wasi/debug/response_transformer.wasm")) 
    end)
