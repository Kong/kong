...
-- XXX: EE [[
for _, workspace in ipairs({ "ws1", "ws2" }) do
-- ]]
...
-- XXX: EE [[
describe("workspace-" .. workspace, function ()
-- ]]
...
describe("Healthcheck #" .. strategy, function()
  lazy_setup(function()
    ...
    if strategy ~= "off" then
      bp.workspaces:insert({ name = workspace })
      bu.setup_prefix("/" .. workspace)
    end
    -- ]]
...
describe("mTLS #" .. strategy, function()
  ...
  it("#db create active health checks -- upstream certificate", function()
    ...
    -- configure healthchecks
    bu.begin_testcase_setup(strategy, bp)
    local certificate_id = bu.add_certificate(bp, {
    ...
    local upstream_name, upstream_id = bu.add_upstream(bp, {
      ...
      client_certificate = { id = certificate_id },
    })
    bu.add_target(bp, upstream_id, "notlocalhost.test", 15555)
    bu.end_testcase_setup(strategy, bp)
...
end  -- for mode, localhost in pairs(bu.localhosts) do

-- XXX: EE [[
end) -- describe("workspace-" .. workspace,
-- ]]

end  -- for _, workspace in ipairs({ "ws1", "ws2" }) do
