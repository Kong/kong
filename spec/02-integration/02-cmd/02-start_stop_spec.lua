...
describe("kong start/stop #" .. strategy, function()
  ...
  it("failure to read referenced secrets when admin_gui_auth_conf set with vault", function()
    ...
    assert.matches("vault://env/auth_conf", stderr, nil, true)
    assert.matches("Error: failed to dereference '{vault://env/auth_conf}'", stderr, nil, true)
  ...
  it("kong startup success when admin_gui_auth_conf set with vault", function()
    ...
    local ok, _, stdout = kong_exec("start", {
      prefix = PREFIX,
      database = strategy,
