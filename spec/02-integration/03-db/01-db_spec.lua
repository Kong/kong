...
for _, strategy in helpers.each_strategy() do
  ...
  describe(":batch() [#" .. strategy .. "]", function()
    lazy_setup(function()
      helpers.get_db_utils(strategy, {})
    end)

    cassandra_only("does not override consistency when explicitly given", function()
      local any = require "cassandra".consistencies.any
      local one = require "cassandra".consistencies.one
      local serial = require "cassandra".consistencies.serial
      ...
      assert.equal(one, opts.consistency)
      assert.equal(serial, opts.serial_consistency)
      ...
      local opts = { consistency = any, serial_consistency = any }
      db.connector:query(queries[1][1], nil, opts, "write")
      assert.equal(any, opts.consistency)
    ...
    cassandra_only("turns on logged batches when requested", function()
      ...
      local opts = {}
      db.connector:batch(queries, opts, "write")
      assert.equal(nil, opts.logged)

      local opts = { logged = false }
      db.connector:batch(queries, opts, "write")
      assert.equal(false, opts.logged)

      local opts = { logged = true }
      db.connector:batch(queries, opts, "write")
      assert.equal(true, opts.logged)

      local opts = {}
      db.connector:batch(queries, opts, "write", false)
      assert.equal(false, opts.logged)

      local opts = {}
      db.connector:batch(queries, opts, "write", true)
      assert.equal(true, opts.logged)
