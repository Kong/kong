    ...
    describe("plugins insert()", function()
      ...
      describe("unique per API/Consumer", function()
        it("API/Plugin", function()
          ...
          assert.matches("["..kong_config.database.." error] name=already exists with value 'key-auth'", tostring(err), nil, true)
        end)
        it("API/Consumer/Plugin", function()
          ...
          assert.falsy(plugin)
          assert.True(err.unique)
          assert.matches("["..kong_config.database.." error] name=already exists with value 'rate-limiting'", tostring(err), nil, true)
        end)
      end)
    end)
