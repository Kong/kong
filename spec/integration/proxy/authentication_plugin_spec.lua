...
describe("Authentication Plugin #proxy", function()
  ...
  describe("Query Authentication", function()
    ...
    it("should pass with POST and hide credentials", function()
      local response, status, headers = http_client.post(STUB_POST_URL, {apikey = "apikey123", wot = "wat"}, {host = "test6.com"})
      assert.are.equal(200, status)
      local parsed_response = cjson.decode(response)
    ...
    it("should pass with GET and hide credentials and another param", function()
      local response, status, headers = http_client.get(STUB_GET_URL, {}, {host = "test6.com", apikey = "apikey123", wot = "wat"})
      assert.are.equal(200, status)
      local parsed_response = cjson.decode(response)
      assert.falsy(parsed_response.headers.apikey)
    ...
    it("should not pass with GET and hide credentials", function()
      local response, status, headers = http_client.get(STUB_GET_URL, {}, {host = "test6.com", apikey = "apikey123123"})
      local body = cjson.decode(response)
      assert.are.equal(403, status)
      assert.are.equal("Your authentication credentials are invalid", body.message)
    end)
