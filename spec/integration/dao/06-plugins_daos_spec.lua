  ...
  describe("Plugins DAOs with DB: #"..db_type, function()
    ...
    describe("plugins migrations", function()
      ...
      setup(function()
        ...
        factory = Factory(db_type, default_options, {"key-auth", "basic-auth", "acl", "hmac-auth", "jwt", "oauth2", "rate-limiting", "response-rate-limiting"})
      end)
      teardown(function()
        f:drop_schema()
      end)
      it("migrations_modules()", function()
        ...
        assert.is_table(migrations["response-rate-limiting"])
      end)
