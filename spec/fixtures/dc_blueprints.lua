...
local function wrap_db(db)
  ...
  for name, _ in pairs(db.daos) do
    dc_as_db[name] = {
      insert = function(_, tbl)
        tbl = kong_table.cycle_aware_deep_copy(tbl)
        ...
        return kong_table.cycle_aware_deep_copy(tbl)
      end,
  ...
  dc_as_db.export = function()
    return kong_table.cycle_aware_deep_copy(config)
  end

  dc_as_db.import = function(input)
    config = kong_table.cycle_aware_deep_copy(input)
  end
