...
describe("Authentication Plugin", function()
  ...
  setup(function()
    ...
    spec_helper.insert_fixtures {
      ...
      plugin = {
        {name = "mm-hmac-auth", config = {clock_skew = 3000}, __api = 1}
      },
      mm_hmacauth_credential = {
  ...
  describe("HMAC Authentication", function()
    ...
    it("should not be authorized when date header is missing", function()
      local response, status = http_client.get(STUB_GET_URL, {}, {host = "hmacauth.com", ["authorization"] = "asd"})
      local body = json_decode(response)
      assert.equal(403, status)
      assert.equal(SIGNATURE_NOT_VALID, body.message)
    end)
    ...
    it("should pass with GET", function()
      ...
      local encodedSignature   = base64_encode(hmac_sha1_binary("secret", stringToSign))
      local hmacAuth = "MM-1 username "..encodedSignature
    ...
    it("should pass with POST", function()
      ...
      local encodedSignature   = base64_encode(hmac_sha1_binary("secret", stringToSign))
      local hmacAuth = "MM-1 username "..encodedSignature
      local response, status = http_client.post(STUB_POST_URL, "['this', 'is', 'the', 'body']", {host = "hmacauth.com",  date = date, authorization = hmacAuth})
      assert.equal(200, status)
      local parsed_response = json_decode(response)
      assert.equal(hmacAuth, parsed_response.headers["authorization"])
    end)
