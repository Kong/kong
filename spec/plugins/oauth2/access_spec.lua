  ...
  describe("OAuth2 Authorization", function()
    ...
    describe("Code Grant", function()
      ...
      it("should work even if redirect_uri contains a query string", function()
        ...
        assert.are.equal(1, utils.table_size(body))
        assert.truthy(rex.match(body.redirect_uri, "^http://google\\.com/kong\\?foo=bar&code=[\\w]{32,32}$"))
      end)
    ...
    describe("Implicit Grant", function()
      ...
      it("should return success and the state", function()
        ...
        assert.are.equal(200, status)
        assert.are.equal(1, utils.table_size(body))
        assert.truthy(rex.match(body.redirect_uri, "^http://google\\.com/kong\\?state=wot&token_type=bearer&access_token=[\\w]{32,32}$"))
      end)
