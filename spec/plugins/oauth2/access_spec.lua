  ...
  describe("OAuth2 Authorization", function()
    ...
    describe("Client Credentials", function()
      ...
      it("should return an error when client_secret is not sent", function()
        local response, status = http_client.post(PROXY_URL.."/oauth2/token", { client_id = "clientid123", scope = "email", response_type = "token" }, {host = "oauth2_4.com"})
        local body = cjson.decode(response)
        assert.are.equal(400, status)
        assert.are.equal(2, utils.table_size(body))
        assert.are.equal("invalid_request", body.error)
      ...
      it("should return an error when client_secret is not sent", function()
        local response, status = http_client.post(PROXY_URL.."/oauth2/token", { client_id = "clientid123", client_secret="secret123", scope = "email", response_type = "token" }, {host = "oauth2_4.com"})
        local body = cjson.decode(response)
        assert.are.equal(400, status)
        assert.are.equal(2, utils.table_size(body))
        assert.are.equal("invalid_request", body.error)
      ...
      it("should return success with authorization header", function()
        local response, status = http_client.post(PROXY_URL.."/oauth2/token", { scope = "email", grant_type = "client_credentials" }, {host = "oauth2_4.com", authorization = "Basic Y2xpZW50aWQxMjM6c2VjcmV0MTIz"})
        local body = cjson.decode(response)
        assert.are.equal(200, status)
        assert.are.equals(4, utils.table_size(body))
        assert.truthy(body.refresh_token)
