...
describe("Plugin: jwt (parser)", function()
  ...
  describe("verify registered claims", function()
    it("requires claims passed as arguments", function()
      ...
      local ok, errors = jwt:verify_registered_claims({"exp", "nbf", "cip"})
      assert.False(ok)
      assert.same({exp = "must be a number", nbf = "must be a number", cip = "must be a string"}, errors)
    ...
    it("checks the type of given registered claims", function()
      local token = jwt_parser.encode({exp = "bar", nbf = "foo", cip = os.time()}, "secret")
      local jwt = assert(jwt_parser:new(token))
    ...
    it("checks the cip claim", function()
      local token = jwt_parser.encode({cip = "127.0.0.1"}, "secret")
      local jwt = assert(jwt_parser:new(token))

      local ok, errors = jwt:verify_registered_claims({"cip"})
      assert.False(ok)
      assert.same({cip = "missing remote_addr"}, errors)

      local args = { remote_addr = "127.0.0.X"}
      local ok, errors = jwt:verify_registered_claims({"cip"}, args)
      assert.False(ok)
      assert.same({cip = "client ip mismatch"}, errors)
    end)
  end)
end)
