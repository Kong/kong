...
local tests_config = json_encode({
  pw_metrics = {
    label_patterns = {
...
local function service_and_route(bp, name, path)
...
for _, strategy in helpers.each_strategy() do
  describe("Plugin: prometheus (metrics) [#" .. strategy .. "]", function()
    ...
    setup(function()
      ...
      local service, _ = service_and_route(bp, "mock", "/")
      local service2, _ = service_and_route(bp, "mock2", "/v2")

      service_and_route(bp, "status.io", "/metrics")

      local filters = {
        { name = "tests", enabled = true, config = tests_config },
      }
      ...
      local res = proxy_client:get("/", {
        headers = { host = "mock", [HEADER_NAME_TEST] = "update_metrics" },
      })
      assert.res_status(200, res)

      res = proxy_client:get("/v2", {
        headers = { host = "mock2", [HEADER_NAME_TEST] = "update_metrics" },
      })
      assert.res_status(200, res)
