...
for _, strategy in helpers.all_strategies() do if strategy ~= "cassandra" then
  describe(PLUGIN_NAME .. ": (access) [#" .. strategy .. "]", function()
    ...
    describe("request", function()
      ...
      it("passes through chat message when allow_untemplated_requests is ON", function()
        local r = client:get("/request", {
          headers = {
            host = "test2.com",
            ["Content-Type"] = "application/json",
      ...
      it("errors with a not found template", function()
        ...
        assert.same(json, { error = true, message = "could not find template name [developer-doesnt-exist]" } )
      end)
      ...
      it("errors with missing template parameter", function()
        ...
        assert.same(json, { error = true, message = "missing template parameters: [program]" } )
      end)
    end)
  end)
