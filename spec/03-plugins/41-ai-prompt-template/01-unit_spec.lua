...
describe(PLUGIN_NAME .. ": (unit)", function()
  ...
  it("templates chat messages", function()
    local rendered_template, err = templater:render(good_chat_template, "llm/v1/chat", templated_chat_request.parameters)
    assert.is_nil(err)
    assert.same(rendered_template, good_expected_chat)
  end)

  it("templates a prompt", function()
    local rendered_template, err = templater:render(good_prompt_template, "llm/v1/completions", templated_prompt_request.parameters)
    assert.is_nil(err)
    assert.same(rendered_template, good_expected_prompt)
  end)

  it("prohibits json injection", function()
    local rendered_template, err = templater:render(good_chat_template, "llm/v1/chat", templated_chat_request_inject_json.parameters)
    assert.is_nil(err)
