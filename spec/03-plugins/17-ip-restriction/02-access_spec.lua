...
for _, strategy in helpers.each_strategy() do
  ...
  describe("Plugin: ip-restriction (access) [#" .. strategy .. "]", function()
    ...
    describe("deny", function()
      it("blocks a request when the IPv6 is denyed", function()
        ...
        local json = cjson.decode(body)
        assert.not_nil(json)
        assert.same("IP address not allowed: ::1", json.message)
      end)
      ...
      it("blocks the IPv6 with CIDR", function()
        ...
        local json = cjson.decode(body)
        assert.not_nil(json)
        assert.same("IP address not allowed: fe80::1", json.message)
      end)
      it("blocks an IPv6 on a allowed IPv6 CIDR range", function()
        local res = assert(proxy_client:send {
          method  = "GET",
  ...
  describe("Plugin: ip-restriction [#pdkconsumergroups] (access) [#" .. strategy .. "]", function()
    local proxy_client
    local admin_client
    ...
    lazy_setup(function()
      ...
      -- Pretend that the ctx.authenticated_consumer_groups is set
      -- via a authentication plugin
      bp.plugins:insert {
        name = "ctx-checker",
        config = {
          ctx_set_field = "authenticated_consumer_groups",
          ctx_set_array_of_maps = { {id = a_consumer_group.id}, {id = b_consumer_group.id} },
          ctx_set_value = nil,
        }
      }
