...
describe("Plugin: datadog (schema)", function()
  ...
  it("rejects null host without env_prefix", function()
    local _, err = v({ host = ngx.null }, schema_def)
    assert.equal("required field missing", err.config.host)
  end)
  it("rejects null port without env_prefix", function()
    local _, err = v({ port = ngx.null }, schema_def)
    assert.equal("required field missing", err.config.port)
  end)
  it("accepts null host and port with env_prefix", function()
    local ok, err = v({ host = ngx.null, port = ngx.null, env_prefix = "SOMEPREFIX" }, schema_def)
    assert.is_nil(err)
    assert.is_truthy(ok)
  end)
