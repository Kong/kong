...
describe("Plugin: response-ratelimiting (access)", function()
  ...
  describe("Without authentication (IP address)", function()
    it("should get blocked if exceeding limit", function()
      ...
      for i = 1, limit do
        ...
        assert.equal(tostring(limit), res.headers["x-ratelimit-limit-video-minute"])
        assert.equal(tostring(limit - i), res.headers["x-ratelimit-remaining-video-minute"])
    ...
    it("should handle multiple limits", function()
      for i = 1, 3 do
        ...
        assert.res_status(200, res)
        assert.equal(tostring(6), res.headers["x-ratelimit-limit-video-minute"])
        assert.equal(tostring(6 - (i * 2)), res.headers["x-ratelimit-remaining-video-minute"])
        assert.equal(tostring(10), res.headers["x-ratelimit-limit-video-hour"])
        assert.equal(tostring(10 - (i * 2)), res.headers["x-ratelimit-remaining-video-hour"])
        assert.equal(tostring(4), res.headers["x-ratelimit-limit-image-minute"])
        assert.equal(tostring(4 - i), res.headers["x-ratelimit-remaining-image-minute"])
      ...
      local body = assert.res_status(429, res)
      assert.equal([[]], body)
      assert.equal("0", res.headers["x-ratelimit-remaining-video-minute"])
      assert.equal("4", res.headers["x-ratelimit-remaining-video-hour"])
      assert.equal("1", res.headers["x-ratelimit-remaining-image-minute"])
    end)
  end)
  ...
  describe("With authentication", function()
    describe("Default plugin", function()
      it("should get blocked if exceeding limit and a per consumer setting", function()
        ...
        assert.equal([[]], body)
        assert.equal("0", res.headers["x-ratelimit-remaining-video-minute"])
        assert.equal("2", res.headers["x-ratelimit-limit-video-minute"])
      end)
      ...
      it("should get blocked if exceeding limit and a per consumer setting", function()
        ...
        assert.res_status(200, res)
        assert.equal("0", res.headers["x-ratelimit-remaining-video-minute"])
        assert.equal("6", res.headers["x-ratelimit-limit-video-minute"])
      end)
  ...
  describe("Upstream usage headers", function()
    it("should append the headers with multiple limits", function()
      ...
      assert.equal("4", body.headers["X-Ratelimit-Remaining-Image"])
      assert.equal("6", body.headers["X-Ratelimit-Remaining-Video"])
      ...
      local body = cjson.decode(assert.res_status(200, res))
      assert.equal("3", body.headers["X-Ratelimit-Remaining-Image"])
      assert.equal("4", body.headers["X-Ratelimit-Remaining-Video"])
    end)
  end)
  ...
  describe("Continue on error", function()
    ...
    it("should not continue if an error occurs", function()
      ...
      assert.res_status(200, res)
      assert.equal("6", res.headers["x-ratelimit-limit-video-minute"])
      assert.equal("5", res.headers["x-ratelimit-remaining-video-minute"])
