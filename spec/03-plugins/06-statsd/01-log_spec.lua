  ...
  describe("Plugin: statsd (log) [#" .. strategy .. "]", function()
    ...
    describe("configures globally", function()
      it("sends default metrics with global.matched namespace", function()
        ...
        ---[[ EE
        assert.not_contains("kong.global.unmatched.cache_datastore_hits_total:[nil%d]+|c", metrics, true)
        assert.not_contains("kong.global.unmatched.cache_datastore_misses_total:[nil%d]+|c", metrics, true)
        --]] EE
      end)
    end)
  end)
