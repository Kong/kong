...
describe("Plugin: statsd (schema)", function()
  ...
  it("accepts valid tag", function()
    local ok, err = validate_entity({ tag = "dogstatsd" }, statsd_schema)
    assert.is_nil(err)
    assert.truthy(ok)
    local ok, err = validate_entity({ tag = "influxdb" }, statsd_schema)
    assert.is_nil(err)
    assert.truthy(ok)
    local ok, err = validate_entity({ tag = "librato" }, statsd_schema)
    assert.is_nil(err)
    assert.truthy(ok)
    local ok, err = validate_entity({ tag = "signalfx" }, statsd_schema)
    assert.is_nil(err)
    assert.truthy(ok)
  end)
  it("rejects invalid tag", function()
    ...
    assert.not_nil(err)
    assert.equal("expected one of: dogstatsd, influxdb, librato, signalfx", err.config.tag)
  end)
end)
