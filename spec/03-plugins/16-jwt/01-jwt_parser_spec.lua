...
describe("Plugin: jwt (parser)", function()
  describe("Encoding", function()
    it("should properly encode using HS256", function()
      ...
      assert.is_true(jwt:verify_signature("secret"))
    end)
    ...
    it("should properly encode using RS256", function()
      ...
      assert.truthy(token)
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.rs256_public_key))
    end)
    it("should properly encode using RS384", function()
      ...
      assert.truthy(token)
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.rs384_public_key))
    end)
    it("should encode using RS512", function()
      ...
      assert.truthy(token)
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.rs512_public_key))
    end)
    ...
    it("should encode using ES256", function()
      ...
      assert.truthy(token)
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.es256_public_key))
    end)
    ...
    it("should encode using ES384", function()
      ...
      assert.truthy(token)
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.es384_public_key))
    end)
  end)
  ...
  describe("verify signature", function()
    it("using HS256", function()
      ...
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature("secret"))
      assert.is_false(jwt:verify_signature("invalid"))
    end)
    ...
    it("using RS256", function()
      ...
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.rs256_public_key))
      assert.is_false(jwt:verify_signature(fixtures.rs384_public_key))
    end)
    it("using RS384", function()
      local token = jwt_parser.encode({sub = "foo"}, fixtures.rs384_private_key, "RS384")
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.rs384_public_key))
      assert.is_false(jwt:verify_signature(fixtures.rs512_public_key))
    end)
    it("using RS512", function()
      local token = jwt_parser.encode({sub = "foo"}, fixtures.rs512_private_key, 'RS512')
      local jwt = assert(jwt_parser:new(token))
      assert.is_true(jwt:verify_signature(fixtures.rs512_public_key))
      assert.is_false(jwt:verify_signature(fixtures.rs256_public_key))
    end)
