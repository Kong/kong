...
describe("Plugin: request-termination (schema)", function()
  ...
  it("should accept echo_trigger", function()
    assert(v({ echo = true, echo_trigger = "header_name" }, schema_def))
  end)
  ...
  describe("errors", function()
    ...
    it("echo_trigger without echo", function()
      local ok, err = v({echo_trigger = "header_name", body = "<body><h1>Not found</h1>"}, schema_def)
      assert.falsy(ok)
      assert.same("echo_trigger can only be used with echo", err.config)
    end)
  end)
end)
